...
In this chapter, a detailed experiment will be conducted, simulating the entire development cycle of a web application using microfrontends architecture across several stages: planning, setup, implementation, build, deployment, testing, as well as continuous integration and deployment. Each stage will be closely monitored to gather comprehensive insights into this architecture. Additionally, a single-page application version of the same application, derived from the microfrontends version, will be implemented to compare performance metrics and evaluate the overall system behavior of both approaches in the next chapter. The primary goal is to provide a deep understanding of the microfrontend approach, highlighting its potential benefits and drawbacks.
...
However, to ensure the experiment is comprehensive, a mocked server that simulates the backend will be set up to handle requests from the microfrontends. While this server could be built using a web framework in any programming language, a JavaScript-based framework will be used to minimize additional setup efforts and maintain consistency with the frontend technologies.

=== Horizontal-Split

The principles of @ddd offer a strong foundation for implementing a vertical-split strategy for microfrontends. In this approach, each microfrontend aligns with a specific subdomain, such as a particular game, effectively mirroring the microservices architecture established on the backend.

However, to increase the system's flexibility, a horizontal-split strategy will be adopted instead. While the majority of microfrontends will continue to follow a vertical-split, focusing on specific game subdomains, certain functionalities or cross-cutting concerns will be shared across multiple microfrontends. For example, although the homepage and Lotto game are developed in separate microfrontends, the component responsible for displaying Lotto quotes can be exposed and reused within the homepage microfrontend. This adoption of a horizontal-split strategy ensures that the architecture remains adaptable and responsive to diverse requirements.
...
For enabling communication between microfrontends, any methods outlined in @fundamental_communication will be effective. These methods can be easily opted in or out, without need for upfront planning.
...
In this experiment a monorepo strategy will be employed. This approach involves storing all microfrontends, UI library, toolings and server application a single repository, simplifying the setup stage, especially within the scope of this experiment.
...
#grid(
 ...
 ,
  [
    - apps: This directory contains the main host, known as shell application (`shell`), which integrates and manges remotes, referred as microfrontends, such as `home` and `lotto`. Each remote, along with the host, is developed and maintained within its own subdirectory.

    - packages: Here stores shared logics and resources, such as the `ui` library or `mfe-config`, an overview configuration file for all microfrontends and the host application. These shared packages ensure consistent styling and functionality throughout the project.

    - server: This folder houses the server application, which acts as a simulated backend, processing requests from the microfrontends.
...
#figure(
  caption: "Vite configuration for both the home and lotto applications"
)[
  ```ts
...
To prevent a single point of failure that could potentially bring down the entire application if the UI library fails, the UI will be bundled within each microfrontend during the compile time, rather than being deployed as a separate microfrontend. This approach ensures that all essential base elements are packaged together, thereby simplifying the management of multiple deployments and minimizing the risk of version inconsistencies. As illustrated in @figure_vite_config_ui, the UI library will be built using the ES module format. Additionally, the Vue package is excluded from the build, as it is already present in both the host and remote applications.
...
Following the selection of the required development tools, this stage focuses on implementing the host application, the two microfrontends (`home` and `lotto`), the UI library, and the server application. Additionally, a routing issue will be identified during this process, and a solution will be developed to address it.
...
After defining the entry component `App.vue`, all necessary routes will be registered as illustrated in @figure_shell_router. The first route is associated with `home` microfrontend for the path `/`, representing the homepage. The second route, mapped to `lotto` microfrontend, corresponds to the path `/lotto6aus49`, where users can participate in lotto games. If no path matches the specified routes, the user is redirected to an error page, which is handled by the `Error.vue` component.
...
#figure(
  caption: "Creation of a router of the host application, along with the registration of three routes."
)[
  ```ts
...
The implementation of each microfrontend is straightforward and aligns with the development of normal single-page application. For instance, the `App.vue` component in the `home` microfrontend might contain a simple heading displaying "Homepage". When a user navigates to the homepage, Vue Router loads this template into the `App` component of the `shell` application, producing the result shown below.

#figure(
  caption: "The content of the home microfrontend is loaded into the shell application's layout when the user navigates to the homepage."
)[
  ```html
  <!-- apps/home/App.vue -->
  <template>
...
As shown in @figure_button_ui_with_customized, a basic `UiButton` component is implemented as a simple HTML button element with predefined Tailwind CSS classes and no context-specific logic. If the home microfrontend requires a customized button, it can create a wrapper around this component to extend its own styles, as demonstrated by the `HomeButton` component.

#figure(
  caption: "The UiButton provides basic styling, while the `HomeButton` extends it with custom styles designed for the homepage."
)[
  ```vue
...
#figure(
  caption: "An Elysia instance is configured as a server, which only allows requests from the host application."
)[
  ```ts
...
Additionally, the `lotto` microfrontend must expose its corresponding components for these new routes, ensuring that the correct components are available to be loaded and displayed by the host application.
...
These routes share the prefix `/lotto6aus49`, which suggests that a separate Vue Router instance should ideally be created within the `lotto` microfrontend to manage its own nested routes. This approach would allow the host application's router to register only the top-level routes for its remotes, while deeper-level routing would be handled within each microfrontend. However, this approach is not feasible under the current Module Federation setup. In this architecture, only a single instance of Vue is created within the host application, which utilizes the router defined in @figure_shell_router. Consequently, no additional Vue or Vue Router instance exists within the `lotto` microfrontend to manage nested routing independently.
...
The initial step in this routing solution is to create an overview configuration for all applications. This overview specifies the directory locations, operating ports, names, and prefixes for each application. This configuration is important not only for defining how each microfrontend is served but also for enabling the host application to access information about its remotes. Henceforth, the term "overview configuration" will refer to this specific configuration.
...
#figure(caption: [A wrapper function is built on top of the `federation` function provided by the plugin `@originjs/vite-plugin-federation`.])[
  ```js
...
The final step in this routing solution is the automated registration of routes. With the overview configuration established in the first step and the details about the exposed components of each microfrontend obtained in the second step, the host application's router can now iterate through the overview configuration, reading the corresponding `routes.json` file and then process compiles a flat array of all possible routes within the application.

#figure(caption: [Creation of a router of the host application with automated routes registration.])[
  ```ts
  const router = createRouter()
...
#figure(caption: [Two unit tests to test the `UiButton` component from UI library.])[
  ```ts
...
#figure(caption: "The code snippet shows the process by which the nginx configurations are generated based on the overview configuration.")[
```ts
// scripts/docker.ts
const cors = `
...
The process of generating Dockerfiles for each microfrontend can be seamlessly integrated into the same loop that creates the Nginx configuration files. The Dockerfile follows a two-stage approach. In the first stage, the necessary files and dependencies are gathered, followed by the build process for the UI library and the specific microfrontend. The second stage sets up the environment for serving the microfrontend. By removing the default configuration, this stage ensures that only custom configurations and assets are used, and it prepares the built assets to be served by the web server on the defined port. Lastly, the command to start the server application by launching the container is specified.

#figure(caption: "The code snippet shows the process by which the Dockerfile is generated based on the overview configuration.")[
  ```ts
  // scripts/docker.ts
  const content = `
  FROM oven/bun:slim AS build
...
The server application's service is first defined, specifying the location of its Dockerfile and the port it will run on. Following this, the overview configuration is looped through again to generate service definitions for each microfrontend. These definitions include the service name, relevant build settings, and necessary port mappings. The script also ensures that each microfrontend service waits for the server to start, maintaining the correct initialization sequence. Once all configurations are defined, they are written into a `docker-compose.yml` file.
...
#figure(caption:"The code snippet shows the process by which the docker-compose.yml is generated based on the overview configuration.")[
  ```ts
  const contents = [
  `services:
      server:
...
In this experiment, both single and multi-container approaches are suitable. However, if the @dklb project later decides to adopt a microfrontend architecture and must select one, it will be essential to carefully weigh the importance of memory efficiency against the flexibility and ease of maintenance.
...
#figure(caption: "Configuration of the pipeline for automating end-to-end testing.")[
  ```yml
...
After completing the code integration, the focus shifts to automated deployment. DigitalOcean has been selected as the provider for the virtual private server (VPS). Significant effort has been invested in generating the necessary Docker configuration files during the deployment stage. The following code snippet demonstrates the action file used to manage the deployment process. This workflow is triggered whenever new code is pushed to the main branch and contains three primary steps: configuring SSH keys, installing the command-line interface for DigitalOcean, and establishing access to the VPS. Once access is secured, a sequence of commands is executed, including pulling the latest code via Git, bringing down existing Docker containers, rebuilding them, and bringing them back up.

#figure(caption: "Configuration of the pipeline for automated deployment.")[
  ```yml
  # .github/workflows/e2e.yml
  # ...
  steps:
