...
This chapter will use the microfrontends decision framework introduced by Luca Mezzalira in his book "Building Microfrontends" @mezzalira_BuildingMicroFrontends2nd_. This comprehensive framework focuses on four key areas: definition, composition, routing, and communication. These areas are important and must be thoroughly considered sequentially and decided upon before starting to build a microfrontends system.

To provide a more rounded perspective and deepen the understanding of this framework, this chapter also uses insights from two other important works in the field. "The Art of Microfrontends" of Florian Rappl @rappl_ArtMicroFrontends_2021 offers valuable additional viewpoints on best practices and advanced techniques. Similarly, Michael Geers's "Microfrontends in Action" @geers_MicroFrontendsAction_2020 provides practical examples and case studies that illustrate the application of the microfrontends framework in real-world scenarios.

== Split Strategies <section_split>

There are two primary approaches to split a frontend application: horizontally or vertically. In both approaches, a microfrontend is defined to represent a complete subdomain, which can be categorized as core, supporting or generic, as introduced in @section_ddd. A horizontal split implies a one-to-many relationship between views and microfrontends, where a single view is made up of multiple microfrontends, each responsible for a specific feature within that view. In contrast, a vertical split establishes a many-to-one relationship, where each microfrontend is responsible for one or more complete vertical slices of the application.

=== Horizontal-Split

In this approach, microfrontends coexist within the same view, each responsible for a specific section of the page. For instance, in a product detail view (as illustrated in @figure_horizontal_split), three microfrontends managed by the product team, the recommendation team, and the checkout team collaborate to present a unified user interface within a single page.

Horizontal-split architecture is often chosen when there is a requirement for reusable microfrontends across different pages within the application. By supporting granular modularization, this method not only enhances reusability and flexibility but also promotes more efficient resource usage. Additionally, this approach proves particularly beneficial in scenarios where multiple teams are working simultaneously on the application, as it allows for the easy division of subdomains into smaller, manageable units.

Since each microfrontend functions as an isolated component of the user interface, any issues that arise within one section do not affect the entire application. This separation mitigates single points of failure, enhances overall stability, and facilitates more seamless updates and maintenance.

Despite its benefits, implementing a horizontal-split architecture presents considerable challenges. It demands strong governance, regular reviews, and effective communication among teams to ensure that each microfrontend maintains its proper boundaries. Additionally, the number of microfrontends within the same view needs to be limited. Over-engineering can lead to an excessive number of small microfrontends, blurring the line between a microfrontend (a business subdomain) and a component (a technical solution for reusability). This can lead to increased overhead, ultimately outweighing the potential advantages.

#figure(image("/assets/horizontal_split_web.png"),
  caption: [A product detail view composed of three microfrontends.]  
) <figure_horizontal_split>

=== Vertical-Split

The vertical-split approach divides an application into multiple slices, with each team managing a specific subset within these slices. For instance, as illustrated in @figure_vertical_split, the recommendation team is responsible for the products overview page, which includes displaying new and recommended products to customers. While the product team handles the product detail view, designing an interface that provides detailed product informations such as descriptions, options and images.

A key component of this architecture is the application shell, which is responsible for loading and unloading microfrontends. It is the first to be downloaded when the application is accessed and remains a persistent part of the application. The shell typically includes essential elements that appear in every page of the application, such as the navigation bar, footer, or general logic as user authentication system.

The vertical-split architecture is particularly beneficial for projects that requires a consistent user interface across different sections. By loading the shell first, users experience a more responsive application, as the core interface elements become immediately available while other parts continue to load in the background. Additionally, each team is responsible for managing the complete user experience for their assigned subdomain, leading to a cohesive and unified interface throughout the application.

Moreover, for teams experienced in developing single-page applications, transitioning to a vertical-split microfrontend architecture is relatively straightforward. The tools, best practices, and design patterns from @spa development are directly applicable, allowing teams to leverage their existing expertise. This continuity reduces the need for extensive retraining and accelerates the implementation process.

Vertical-splits also bring notable challenges that require careful attention. A primary concern is the potential for redundant efforts and code duplication. Without careful management and coordination, individual microfrontends within a vertical-split architecture might independently develop its own versions of common functions, such as data fetching or error handling. This redundancy not only increases the maintenance workload but also results in a larger, more complicated codebase. Managing these redundant components can become a complex and time-consuming task, reducing the efficiency that vertical-split architecture is intended to achieve.

Moreover, since a microfrontend represents multiple slices within the application, its failure could potentially impact a large number of users. For instance, in an online shopping application, as illustrated in @figure_vertical_split, if the product detail page encounters an issue or becomes unavailable, customers would be unable to view products or make purchases. This could result in significant sales losses and a negative user experience.

#figure(image("/assets/vertical_split_web.png"),
  caption: [User workflow between two microfrontends.],
) <figure_vertical_split>

== Composition and Routing

There are three primary composition types: server-side composition, edge-side composition, and client-side composition. The definition of a microfrontend affects how they are composed into a view. Vertical-split architecture supports only client-side composition, whereas horizontal-split architecture can accommodate all three types. These composition methods correspond to three routing approaches, determined by where the routing logic is executed: server-side routing, edge-side routing, and client-side routing. Before exploring the details of each composition and routing mechanism, the different between build-time integration and runtime integration will first be discussed.
 
=== Build-time vs. Runtime Integration

Build-time integration refers to the process where microfrontends are integrated during the build phase. In this approach, individual microfrontends are combined into a single deployable bundle before the application is served to the user. While this method allows for independent during the development stage, it results in coupling at the release stage. Consequently, even a minor change in a single microfrontend may necessitate recompiling the entire application, creating a distributed monolith architecture that contradicts the fundamental principles of microfrontend architecture.

In contrast, runtime integration involves assembling the microfrontends dynamically when the application is loaded or accessed by the user. This approach helps avoid issues such as lock-step deployment, thereby maintaining the independence of each microfrontend. Given these considerations, runtime integration is generally preferred over build-time integration to prevent unnecessary coupling.
...
As an alternative to the server-side method, the edge-side composition and routing occurs at the edge of the network, typically within a @cdn. This approach enables microfrontends to be dynamically assembled as close to the user as possible.
...
It is also important to note that the use cases for edge-side composition are limited and often used along with other patterns, such as server-side composition. While this method is particularly effective for applications requiring rapid delivery and rendering times, its implementation can be complex. Edge-side composition demands advanced infrastructure and expertise to manage effectively, which is why relatively few developers choose to implement microfrontends using this approach, as indicated by a survey on microfrontends conducted in late 2023 @steyer_ConsequencesMicroFrontends_2023.
...
Microfrontends are designed to be independent, modular, and loosely coupled, ideally operating without the need for communication with each other. However, in practice, especially in horizontally-split applications, multiple microfrontends within a single view often need to interact to ensure the correct actions are displayed when a user interacts with the page or to exchange data. This requirement for communication can increase coupling and risk undermining the principle of independent deployment if not implemented carefully. Below are common strategies for enabling communication between microfrontends, each with its own specific use cases and considerations:

=== Query Parameters

Using query parameters to pass state through the URL is an effective method for enabling communication between microfrontends. This approach preserves the independence of each microfrontend by limiting their interaction to the URL. The visibility of state in query strings allows users to bookmark or share the page, enhancing the overall user experience. However, this method is most appropriate for handling simple, serializable data types. Overusing query parameters can lead to excessively long and unwieldy URLs. Therefore, this technique should be reserved for simple and transient state sharing between microfrontends, keeping URLs concise and meaningful to avoid unnecessary clutter.
...
Web storage and cookies both store data in the browser, allowing any microfrontend to access and share data efficiently. There are two types of saved data: temporary and persistent.
...
However, personalized or sensitive data, such as information from an authentication microfrontend, stored in web storage or cookies must be encrypted to ensure security. Careful management is also required to prevent conflicts and ensure data integrity. By using web storage and cookies effectively, microfrontends can maintain their independence while sharing necessary information, thereby enhancing the overall functionality and user experience of the application.

For example, as illustrated in the figure below, two microfrontends can communicate through cookies during a login process. Once the user successfully signs in, the authentication token is encrypted and stored in a cookie. The microfrontend by team user can then retrieve and decrypt this token to verify the user's authentication status and display the appropriate user data.

#figure(
  image("/assets/storage_cookies.png"),
  caption:  [Two microfrontends communicate through cookies during a login process.]
)

=== Event Bus

Another strategy is the event bus, which enables communication between different microfrontends without requiring direct awareness of each other. The event bus operates through two primary mechanisms: event emission and event subscription.

During the event subscription phase, a microfrontend subscribes to specific events on the event bus. The event bus then notifies the subscribing microfrontend whenever another microfrontend emits one of the subscribed events to the bus. For example, in @figure_horizontal_split, when a user selects a different shirt color, the product team emits an event to notify the checkout team that the shirt configuration has changed. As a result, the checkout team will update the price accordingly.

=== Global State Manager

A global state manager becomes specially useful when client-side composition is used. Most frontend frameworks offer state manager libraries, such as Redux #footnote[https://redux.js.org/] for React.js or Pinia #footnote[https://pinia.vuejs.org/] for Vue.js. Similar to communication methods like web storage or an event bus, these tools enable components within a view to communicate more easily and effectively. By centralizing the state, a global state manager allows components to access and update shared states reactively and seamlessly, simplifying the data flow and ensuring a more consistent user experience. However, it relies on web storage to persist data, as the global state is reset upon reloading the page.
