...
A key component of this architecture is the application shell, which is responsible for loading and unloading micro frontends. It is the first to be downloaded when the application is accessed and remains a persistent part of the application. The shell typically includes essential elements that appear in every page of the application, such as the navigation bar, footer, or general logic as user authentication system.
...
As an alternative to the server-side method, the edge-side composition and routing occurs at the edge of the network, typically within a @cdn. This approach enables micro frontends to be dynamically assembled as close to the user as possible.
...
Micro frontends are designed to be independent, modular, and loosely coupled, ideally operating without the need for communication with each other. However, in practice, especially in horizontally-split applications, multiple micro frontends within a single view often need to interact to ensure the correct actions are displayed when a user interacts with the page or to exchange data. This requirement for communication can increase coupling and risk undermining the principle of independent deployment if not implemented carefully. Below are common strategies for enabling communication between micro frontends, each with its own specific use cases and considerations:
